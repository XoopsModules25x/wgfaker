...
function wgfakerMetaKeywords($content)
{
...
function wgfakerMetaDescription($content)
{
    global $xoopsTpl, $xoTheme;
    $myts = MyTextSanitizer::getInstance();
    $content = $myts->undoHtmlSpecialChars($myts->displayTarea($content));
 ...
 */
function wgfaker_RewriteUrl($module, $array, $type = 'content')
{
    $comment = '';
    ...
    if (isset($array['topic_alias']) && $array['topic_alias']) {
        ...
        $topic_name = wgfaker_Filter(xoops_getModuleOption('static_name', $module));
    }
 ...
 * @return string $url
 */
function wgfaker_Filter($url, $type = '') {
